...
impl BatchWriterCache {
    fn global() -> &'static Self {
        ...
        CACHE.get_or_init(|| BatchWriterCache {
    ...
    fn get_or_create<T: Callback + 'static>(&self) -> Arc<BatchWriter<T>> {
        ...
        if let Some(existing) = self.instances.get(&type_id) {
            if let Some(weak_tuple) = existing.downcast_ref::<WeakTuple<T>>() {
        ...
        // 创建新实例
        let (writer, handle) = BatchWriter::<T>::new(None, None, None);
        add2pool(handle);
...
// 用于存储弱引用和任务句柄的辅助结构
struct WeakTuple<T: Callback>(Weak<BatchWriter<T>>);
